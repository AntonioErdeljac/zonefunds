...
@section('content')
  ...
  @endif
    ...
    <div class="container mt-3">
      <div class="row">
        <div class="{{ $ad_select->ad_image != 'default.jpg' ? 'col-lg-4' : 'col-lg-6'}}">
          <div class="card">
            <div class="card-block"  style="color: #2d89e5">
              <h4 class="card-title linebreak">{{$ad_select->ad_title}}</h4>
              <hr>
              <p style="color: #6cace9">{{$ad_select->created_at->diffForHumans()}}</p>
              <p class="pt-1 linebreak">{{$ad_select->ad_body}}</p>
              
              <hr>

              <a class="" style="border-radius: 0px; text-decoration: none; color:#2d89e5; border-right-style:hidden; border-left-style: hidden;" href="callto:{{ $ad_select->ad_contact }}"><i class="fa fa-phone fa-fw" aria-hidden="true"></i>&nbsp; {{ $ad_select->ad_contact }}</a>
              <hr>
              <p class="" style="border-radius: 0px; text-decoration: none; color:#2d89e5; border-right-style:hidden; border-left-style: hidden;" href="#"><i class="fa fa-money fa-fw" aria-hidden="true"></i>&nbsp; {{ $ad_select->ad_price }} HRK</p>
              <hr>
              <!--<p class="" style="border-radius: 0px; text-decoration: none; color:#2d89e5; border-right-style:hidden; border-left-style: hidden;" href="#"><i class="fa fa-map-marker fa-fw" aria-hidden="true"></i>&nbsp; {{ $ad_select->ad_location }}</p>
              <hr>-->
                <a class="" style="border-radius: 5px; text-decoration: none; color:#2d89e5; border-right-style:hidden; border-left-style: hidden;" href="#"><i class="fa fa-user fa-fw" aria-hidden="true"></i>{{ $ad_select->user['username'] }}</a>
        ...
        <div class="{{ $ad_select->ad_image != 'default.jpg' ? 'col-lg-4' : 'col-lg-6'}}" style="color:#2d89e5">
        ...
        <div class="{{ $ad_select->ad_image != 'default.jpg' ? 'col-lg-4' : 'col-lg-6'}}">
                  @if($ad_select->ad_image != 'default.jpg')
            ...
            <div class="card" style="color: #2d89e5;">
              <div class="card-block">
                ...
                <hr>
                <img class="img-fluid" src="image/{{ $ad_select->ad_image }}" style="border-top-right-radius: 5px; border-top-left-radius: 5px;">
              </div>
            </div>
          </div>
              <hr>
...
<div id="createmodal" class="overlay_update text-xs-center">
    <form id="createad_form" action="{{ route('ad.update', $ad_id = $ad_select->id) }}"  method="POST" enctype="multipart/form-data" class="form-control bg-color-niceblue" style="border-style:none;">
        ...
        <div id="final_update" class="">
          <div >
            <input type="text" id="final_title" class="mb-1 ad_create form-control" style="background-color:#2d89e5;" name="ad_title" placeholder="Naslov oglasa" value="{{ $ad_select->ad_title }}"/>
          </div>
          <div>
            <textarea type="text" id="final_body" class="mb-1 ad_textarea form-control" style="background-color:#2d89e5;" name="ad_body" placeholder="Opis oglasa">{{ $ad_select->ad_body }}</textarea>
          </div>
          <div>
              <input type="number" id="final_contact" class="mb-1 ad_create form-control" style="background-color:#2d89e5;" name="ad_contact" placeholder="Kontakt" value="{{ $ad_select->ad_contact }}"/>
          </div>
          <div >
              <input type="number" id="final_price" class="mb-1 ad_create form-control" style="background-color:#2d89e5;" name="ad_price" placeholder="Cijena" value="{{ $ad_select->ad_price }}"/>
          </div>
          <div>
        ...
        <a style="" clas><button type="button" id="close_update" style="cursor:pointer; outline: none;border-radius: 50%; border-color:#fff; border-style:solid; border-width:3px;width: 50px; height: 50px;  background-color: #2d89e5;" class="rotate_half"><i class="fa fa-times" style="color:#fff; "></i></button></a>
         ...
         <a style="" href="{{ route('ad.delete', $ad_select->id) }}"" clas><button type="button" id="" style="cursor:pointer; outline: none;border-radius: 50%; border-color:#fff; border-style:solid; border-width:3px;width: 50px; height: 50px;  background-color: #2d89e5;" class="rotate_half"><i class="fa fa-trash" style="color:#fff; "></i></button></a>
...
<div id="completemodal" class="overlay_complete text-xs-center">
    <form id="createad_form" action="{{ route('ad.complete', $id = $ad_select->id) }}"  method="get" class="form-control bg-color-niceblue" style="border-style:none;">
        ...
        <div id="final_complete" class="">
          <div >
            ...
            <input type="hidden" value="{{ $ad_select->id }}" name="ad_id">
...
<script type="text/javascript">
    ...
    function initMap()
    {
      ...
      infowindow_{{$ad_select->id}} = new google.maps.InfoWindow({
        content:'<a href="{{ route("ad.show", $ad_select->id) }} style="text-decoration: none; !important"><div class="container><div class="row"><h5 class="linebreak">{{ $ad_select->ad_title }}</h5><p style="color: #6cace9"><i>{{$ad_select->created_at->diffForHumans()}}</i></p></span><p class="card-text pt-1 linebreak mb-1">{{ str_limit($ad_select->ad_body, 50) }}</p></div><div class="list-group" style="border-radius: 0px;"><p class="list-group-item" style="border-radius: 0px; text-decoration: none;  border-right-style:hidden; border-left-style: hidden;" href="tel:{{ $ad_select->ad_contact }}"><i class="fa fa-phone fa-fw" aria-hidden="true"></i>&nbsp; {{ $ad_select->ad_contact }}</p><p class="list-group-item" style="border-radius: 0px; text-decoration: none; color:#2d89e5; border-right-style:hidden; border-left-style: hidden;"><i class="fa fa-money fa-fw" aria-hidden="true"></i>&nbsp; {{ $ad_select->ad_price }}</p><p class="list-group-item" style="border-radius: 0px; text-decoration: none; color:#2d89e5; border-right-style:hidden; border-left-style: hidden;"><i class="fa fa-user fa-fw" aria-hidden="true"></i>&nbsp; {{ $ad_select->user["username"] }}</p></a></div></div></div></div></div></div>'
        });


      oznaka.addListener('click', function(){
        infowindow_{{ $ad_select->id }}.open(map, oznaka);
      })
