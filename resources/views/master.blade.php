                ...
                <div class="modal-content border-r-15">
                  <div class="modal-body text-xs-center border-r-15 color-niceblue">
                    <h1 class="display-5">Vaša Lokacija:</h4>
                      <h2 class="display-5" style="{{ $api == 'da' ? 'color:#E54444;' : '' }}"><i class="fa fa-map-marker" style="padding-right: 10px; {{ $api == 'da' ? 'color:#E54444;' : '' }}"></i>{{ $city }}</h2>
                    <hr>
                    <h5 style="color: #E54444;">{{$api == 'da' ? 'Molimo dopustite geolokaciju u vašem pregledniku' : ''}}</h5>
                ...
                <div class="row">
                  <div class="col-lg-12">
                    <div class="form-group">
                       ...
                       <a href="{{ route('logout') }}"> <button class="form-control btn btn-primary border-r-0 mb-1"><i class="fa fa-sign-out fa-fw mr-1" aria-hidden="true"></i>Logout</button> 
                       </a>
                        </div>
                      </form>
                    </div>
                    ...
                    <div class="form-group">
                      <form action="{{route('signin')}}" method="post">
                        ...
                        <div class="input-group mb-1">
                          <input class="form-control input_form input_focus_change" type="text" name="email" placeholder="john.doe@example.com" value="{{ Request::old('email') }}">
                        </div>
                    ...
                    <div class="form-group">
                      <form action="{{ route('signup') }}" method="post">
                        <div class="input-group mb-1">
                          ...
                          <input class="form-control input_focus_change" type="text" name="username" placeholder="John Doe" style="{{ $errors->has('username') ? 'border-color: red; color:red;' : 'border-color: #f0f0f0; border-radius:5px; background-color:#f7f7f7;'}}" value="{{ Request::old('username') }}">
                        </div>

                        <div class="input-group mb-1">
                          <input class="form-control input_focus_change" type="text" name="email" placeholder="john.doe@example.com" style="{{ $errors->has('email') ? 'border-color: red; color:red;' : 'border-color: #f0f0f0; border-radius:5px; background-color:#f7f7f7;'}}" value="{{ Request::old('email') }}">
                        </div>

                        <div class="input-group mb-1">
                      
                          <input class="form-control input_focus_change" type="password" name="password" placeholder="****" style="{{ $errors->has('password') ? 'border-color: red; color:red;' : 'border-color: #f0f0f0; border-radius:5px; background-color:#f7f7f7;'}}">
                        </div>
...
<div id="createmodal" class="overlay text-xs-center">
    <form id="createad_form" action="{{ route('ad.create') }}"  method="POST" enctype="multipart/form-data" class="form-control bg-color-niceblue" style="border-style:none;">
      ...
      $(document).ready(function()
        {
          ...
          @if(Auth::check())
            ...
            console.log('{{ $ad_count }}');
              function getNotifications()
              {
                ...
                $.ajax({
                  type: 'POST',
                  url: '{{ route("postNotifications") }}',
                  success: function(data)

                  {
                    console.log(data.user_money);
                    console.log({{$user_money}});
                    if({{ $ad_count }} != data.notif || {{$user_money}} != data.user_money)
                    {
                      if(data.notif != {{$ad_count}})
                      {
                        new_notifications = data.notif - {{ $ad_count }};
                      }
                      
                      if(data.user_money != {{ $user_money }})
                      {
                        console.log('tu smo');
                      ...
                      if(new_notifications > 0)
                      {
                        ...
                        {
                          $('#notif_show').html('<a href="{{ route("home") }}">'+new_notifications+'&nbsp;Novi Oglas</a>');
                        }
                        else
                        {
                          $('#notif_show').html('<a href="{{ route("home") }}">'+new_notifications+'&nbsp;Nova Oglasa</a>');
                        }
          ...
          {
            $('#notifikacije').click(function(){location.href="{{ route('home') }}"});
          }*/
